...
export class SupportPage extends WebActions {
  ...
  async moveGuidePageAndTextCheck(): Promise<boolean> {
    try {
      // 이용 가이드
      await this.clickMovePage(supportLocator.guide[this.uiType]);
      await this.checkURL(urlLocator.use_guide[this.platform]);
      await this.checkText(supportLocator.guideTab[this.uiType], TextConstants.SUPPORT_GUIDE2);
      return true;
    } catch (err) {
