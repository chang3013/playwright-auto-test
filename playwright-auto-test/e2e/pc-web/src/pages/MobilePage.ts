...
export class MobilePage extends WebActions {
  ...
  async themaBannerSectionCheck(): Promise<boolean> {
    try {
      ...
      const bannerHref: Record<number, string> = {
        ...
        2: urlLocator.usim[this.platform],  // 유심
  ...
  async submenuSectionCheck(): Promise<boolean> {
    try {
      ...
      await this.checkText(mobileLocator.subMenuTabList[this.uiType], TextConstants.MOBILE_SUB_MENU);
      return true;
    } catch (err) {
  ...
  async selectConditionAndMoveCartPage(): Promise<boolean> {
    try {
      ...
      // 사은품
      const gift1 = await this.getLocator(mobileCartLocator.gift[this.uiType]).nth(0).locator('.text-radio');
      await this.cartSelect(gift1);
      // 매월 이용할 혜택
      const gift2 = await this.getLocator(mobileCartLocator.gift[this.uiType]).nth(1).locator('.text-radio');
      await this.cartSelect(gift2);
      // 제휴카드
      if (await this.getLocator(mobileCartLocator.benefit2[this.uiType]).count() > 0){
        await this.cartSelect(await this.getLocator(mobileCartLocator.partnerCard[this.uiType]).nth(9).locator('.text-content'));
      } else {
        await this.cartSelect(await this.getLocator(mobileCartLocator.partnerCard[this.uiType]).nth(8).locator('.text-content'));
      }
    ...
    try {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (await delBtn.first().isVisible()) {
          ...
          const checkBtn = await this.getLocator(uiLocator.modalFooter).locator('button').first();
  ...
  async planInfoCollectAndMoveChangePage(): Promise<boolean> {
    try {
      ...
      // 미디어 혜택
      else if ((await this.getLocator(uiLocator.modalHeader).innerText()).includes('미디어')) {
      ...
      // 프리미엄 혜택
      else if ((await this.getLocator(uiLocator.modalHeader).innerText()).includes('프리미엄')) {
  ...
  async planInfoCollectAndMoveComparePage(): Promise<boolean> {
    try {
      ...
      if (await this.getLocator(uiLocator.modalContent).count() > 0) {
        await this.click(mobilePlanLocator.movePlanCompareBtn[this.uiType]);
      }
