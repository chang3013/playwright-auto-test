...
import { type CDPSession, chromium, type Page, type Browser as PWBrowser } from '@playwright/test';
...
export class CDPConnectUtils {
  ...
  public static async connectToWebView(
    wsEndpoint: string,
    ...
    for (let i = 0; i < this.MAX_RETRIES; i++) {
      try {
        ...
        const msg = err?.message || String(err);
        this.logger?.warn(
          `[CDPConnectUtils][${this.poc}] CDP 연결 시도 실패 (${i + 1}/${this.MAX_RETRIES}): ${msg}`,
        );
        await new Promise(res => setTimeout(res, this.RETRY_DELAY_MS));
      }
    }
    ...
    if (!browser) {
      throw new Error(
        `[CDPConnectUtils][${this.poc}] CDP 연결에 ${this.MAX_RETRIES}회 시도했지만 실패했습니다.`,
      );
    }

    // context 타입 명시 + 필터링
    const context: BrowserContext | undefined = browser
      .contexts()
      .find((ctx: BrowserContext) => ctx.pages().length > 0);
    if (!context) {
    ...
    try {
      ...
      this.logger?.warn(
        `[CDPConnectUtils][${this.poc}] CDP 세션 생성 실패 (WebView 제한 가능성): ${e}`,
      );
      // 세션 없이도 fallback 가능
      return { page };
    }
