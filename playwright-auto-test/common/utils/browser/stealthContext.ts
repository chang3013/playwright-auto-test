  ...
  public async createContext(browser: Browser): Promise<BrowserContext> {
    ...
    if (!userAgent || !viewport) {
      switch (platform) {
        ...
        case 'MOBILE_WEB':
          userAgent ??=
            'Mozilla/5.0 (Linux; Android 13; SM-S916N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Mobile Safari/537.36';
    ...
    await context.addInitScript(
      (env: { viewport?: { width: number; height: number }; isMobile: boolean }) => {
        ...
        Object.defineProperty(navigator, 'plugins', {
          get: () => {
            const fakePlugin = (name: string, filename: string, description: string) => ({
              name,
              filename,
              description,
              length: 1,
