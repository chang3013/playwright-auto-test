...
export class ChromeSetup {
  ...
  public async syncChromedriver(): Promise<void> {
    ...
    try {
      ...
      const { version: resolvedVersion, url: chromedriverUrl } = await this.resolveChromedriverUrl(
        majorVersion,
        platformFolder,
      );
      ...
      await pipeline(response.body as NodeJS.ReadableStream, fs.createWriteStream(downloadPath));
  ...
  private async resolveChromedriverUrl(
    ...
    try {
      ...
      const download = matched.downloads.chromedriver.find(d => d.platform === platformFolder);
      if (!download) {
        throw new Error(
